...
export async function GET(request: NextRequest) {
    try{
        ...
        const baseUrl = "https://tokens.rubic.exchange/api/v1/tokens"
        ...
        const res = await fetch(`${baseUrl}/?network=${network ? (network === "bsc" ? "binance-smart-chain" : (network === "eth" ? "ethereum" : (network === "matic" ? "polygon" : ""))) : "binance-smart-chain"}&address=${address}`, {
            next: { revalidate: 60 } 
        })
        ...
        return NextResponse.json({ status: true, message: "This is token", info: [{title: name, ticker: symbol, decimal: decimals, address: address, img: result["results"][0]["image"]}] });
    } catch (err) {
