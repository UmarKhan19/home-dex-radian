...
export function WebNavigation() {
	...
	return (
		<div
			className={`flex gap-3 items-center w-fit cursor-pointer visible lg:hidden`}
		>
			<Sheet>
				...
				<SheetContent
					side={`top`}
					className={`min-h-[100vh] gradient-header items-center`}
				>
					<SheetHeader>
						<SheetTitle>
								...
								<p
									...
									className={`${
										pathname === item.link
											...
											: "opacity-60"
									} justify-center text-center text-2xl flex items-center gap-3 my-4 cursor-pointer hover:font-bold hover:opacity-100 hover:underline decoration-2 underline-offset-4`}
								>
									{`${item.title}`}
								</p>
							) : item.type === "external" ? (
