...
export default function BridgeTokenSeletor({
    tokenAddress,
    tokenIcon,
    tokenName,
    tokenSymbol,
    ...
    const updateToken = (val: string) => {
        ...
        tokenInfo = tokens["bsc"].find((item) => item.address === val)
        name = tokenInfo.name
        symbol = tokenInfo.symbol
        icon = tokenInfo.icon
        updateTokenAddress(val, name, symbol, icon)
        setToken("")
    }
    ...
    const updateTokenWhitelist = (val: string, title: string, ticker: string, img: string) => {
        ...
        const symbol = ticker
        const icon = img
        updateTokenAddress(val, title, ticker, img)
        setToken("")
    }
            ...
            <DialogTrigger asChild>
                <div className={`cursor-pointer hover:bg-accent hover:text-accent-foreground inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 p-0`}>
                    <div className={`flex gap-2 items-center w-fit`}>
                        {tokenIcon ? (
                            <img src={`${tokenIcon}`} alt={`${tokenName}`} className={`max-w-6 max-h-6`} />
                        ) : (
                            <div className={`max-w-6 max-h-6 rounded-full flex items-center justify-center`}>
                                {tokenIcon && tokenName.char(0).toUpperCase()}
                            </div>
                        )}
