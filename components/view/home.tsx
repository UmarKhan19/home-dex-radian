...
export default function Home() {
  ...
  return (
    <section
      className={`flex-row text-center justify-center items-center min-h-[60vh]`}
    >
      <div className={`grid gap-4 md:gap-8 grid-cols-1 lg:grid-cols-2 my-8`}>
        <div className={`flex lg:hidden h-[50vh] max-h-[240px]`}></div>
        <div
          className={`w-full grid grid-cols-1 gap-8 mt-8 mb-10 justify-center lg:justify-start`}
        >
          {section_1.map((item, index) => sectionOneProcessor({ item, index }))}
          <ButtonBase
            title={
              <a
                href="https://raiden-dex.vercel.app/"
                target="_blank"
      ...
      <div
        className={`grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 my-8`}
      >
...
function sectionOneProcessor({ item, index }) {
  return item.type === "header" ? (
      ...
      {item.content.map((header, idx) =>
        header.type === "underline" ? (
          <span
            ...
            className={`mr-1 gradient-underline`}
          >
            {`${header.content}`}
          </span>
        ) : (
    ...
    <p key={`text-${index}`} className={`text-xl text-left`}>
      {item.content.map((header, idx) =>
        header.type === "underline" ? (
          <span
            key={`header-${index}-${idx}`}
...
function cardProcessor(item) {
  return (
    <div
      className={`grid grid-cols-1 gap-2 justify-center items-center h-full`}
    >
      <div className={`mx-auto`}>
        <img src={item.icon} alt={item.headline} />
      </div>
      <div
        className={`items-center grid grid-cols-[1fr_2fr] gap-4 w-fit mx-auto`}
      >
